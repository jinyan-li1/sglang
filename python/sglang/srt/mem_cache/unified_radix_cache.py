...
class UnifiedRadixCache(BasePrefixCache):
    ...
    def prefetch_from_storage(
        ...
        if req_id in self.ongoing_prefetch or (
            self.buffer_pipeline is not None and self.buffer_pipeline.has_staged(req_id)
        ):
            return
    ...
    def _handle_storage_prefetch_anchor_loss(self, req_id: str) -> None:
        ...
        self.revoke_pending_prefetch(req_id)
        self.storage_prefetch_retries.refetch(req_id, storage_hit_end)
    ...
    def _trim_buffer_prefetch_full_head(
        ...
        prefix_ctx = self.buffer_pipeline._prefetch_prefix_ctx[req_id]
        ...
        self.buffer_pipeline._prefetch_prefix_ctx[req_id] = (
            new_prefix_tokens,
            extra_key,
            cache_salt,
        )
        ...
        self.ongoing_prefetch[req_id] = info
        self.cache_controller.trim_prefetch_full_head(operation, trim_tokens)
        self._resolve_storage_prefetch_tokens(
            req_id, trim_tokens, reason="device_covered"
        )
        ...
        def _try_alloc_storage_hit(operation) -> bool:
            ...
            if buffer_mode:
                ...
                if self.buffer_pipeline.staged_span_covered(
                    ...
                    self._finish_storage_prefetch(
                        req_id, fulfilled_tokens=0, reason="device_covered"
                    )
                    self.revoke_pending_prefetch(req_id)
                    return True
                info, hit_tokens, aux_hit_tokens = self._trim_buffer_prefetch_full_head(
                    req_id, info, operation, full_match_len, hit_tokens
                )
            else:
            ...
            if host_indices is None:
                ...
                self.storage_prefetch_retries.poll_miss(
                    req_id, operation.storage_start + operation.storage_hit_count
                )
                self._log_storage_prefetch_deferred(
                    max(alloc_len, aux_hit_tokens), "host_capacity"
                )
